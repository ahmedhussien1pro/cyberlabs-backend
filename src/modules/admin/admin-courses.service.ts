...
export class AdminCoursesService {
  ...
  async getStats() {
    ...
    return { data: { total, published, draft, comingSoon, featured, unpublished: total - published, byState } };
  }
  ...
  async findAll(query: AdminCourseQueryDto) {
    ...
    if (query.isPublished !== undefined)
      where.isPublished = query.isPublished === true || query.isPublished === ('true' as any);
    ...
    const [rows, total] = await Promise.all([
      this.prisma.course.findMany({ where, skip, take: limit, orderBy: { createdAt: 'desc' }, select: COURSE_LIST_SELECT }),
      this.prisma.course.count({ where }),
    ]);
    return { data: rows.map(normalizeCourse), meta: { total, page, limit, totalPages: Math.ceil(total / limit) } };
  }
  ...
  async findOne(idOrSlug: string) {
    ...
    let raw: any = null;
    if (isUuid) raw = await (this.prisma.course as any).findUnique({ where: { id: idOrSlug }, select: COURSE_FULL_SELECT });
    if (!raw) raw = await (this.prisma.course as any).findUnique({ where: { slug: idOrSlug }, select: COURSE_FULL_SELECT });
    if (!raw) throw new NotFoundException(`Course "${idOrSlug}" not found`);
    return { data: normalizeCourse(raw) };
  }

  // ── Get Curriculum — DB only ────────────────────────────────────
      ...
      select: {
        sections: {
          ...
          select: {
            ...
            lessons: {
              orderBy: { order: 'asc' as const },
              // sentinel (order=0) stores the full elements JSON — include it
              select: {
                id: true,
                title: true,
                ar_title: true,
      ...
      if (sentinel?.content) {
        try {
          ...
          if (Array.isArray(parsed)) {
            ...
            elements = buildElementsFromLessons(lessons);
          }
        } catch {
          elements = buildElementsFromLessons(lessons);
        }
  ...
  async saveCurriculum(idOrSlug: string, rawTopics: object[]) {
    ...
    await (this.prisma.course as any).update({
      where: { id: courseId },
      data: { topics: topicTitles, ar_topics: arTopicTitles, totalTopics: topicTitles.length },
    });
    ...
    for (let sIdx = 0; sIdx < (rawTopics as any[]).length; sIdx++) {
      ...
      const sTitle = typeof topic.title === 'object' ? (topic.title?.en ?? `Section ${order}`) : (topic.title ?? `Section ${order}`);
      const sTitleAr = typeof topic.title === 'object' ? topic.title?.ar : undefined;

      const [section, mod] = await Promise.all([
        this.prisma.section.create({ data: { courseId, title: sTitle, ...(sTitleAr ? { ar_title: sTitleAr } : {}), order } }),
        (this.prisma as any).module.create({ data: { courseId, title: sTitle, ...(sTitleAr ? { ar_title: sTitleAr } : {}), order } }),
      ]);

      const elements: any[] = topic.elements ?? [];

      // sentinel lesson (order=0): stores full elements JSON — getCurriculum reads from here
      ...
      // individual VIDEO lesson rows for progress tracking
      for (let eIdx = 0; eIdx < elements.length; eIdx++) {
        const el = elements[eIdx];
        if ((el.type ?? '').toString().toUpperCase() !== 'VIDEO') continue;
        const lessonOrder = safeNumber(el.order, eIdx + 1);
        const lTitle = typeof el.title === 'object' ? (el.title?.en ?? `Lesson ${lessonOrder}`) : (el.title ?? `Lesson ${lessonOrder}`);
    ...
    return { data: { topics: topicTitles, ar_topics: arTopicTitles, totalTopics: topicTitles.length } };
  }
  ...
  async create(dto: CreateCourseDto) {
    ...
    const course = await this.prisma.course.create({ data: { ...dto, slug, state, isPublished: isPublishedFromState(state) } as any });
  ...
  async update(idOrSlug: string, dto: UpdateCourseDto) {
    ...
    if (updateData.state !== undefined) updateData.isPublished = isPublishedFromState(updateData.state);
    if (updateData.isPublished !== undefined && updateData.state === undefined)
      updateData.state = updateData.isPublished ? 'PUBLISHED' : 'DRAFT';
    const updated = await (this.prisma.course as any).update({ where: { id: existing.id }, data: updateData, select: COURSE_FULL_SELECT });
  ...
  async updateCurriculum(idOrSlug: string, dto: UpdateCurriculumDto) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const [si, section] of sections.entries()) {
        const sec = await (tx as any).section.create({ data: { courseId: id, title: section.title, order: section.order ?? si } });
  ...
  async publish(idOrSlug: string) {
    const { data: existing } = await this.findOne(idOrSlug);
    const updated = await (this.prisma.course as any).update({ where: { id: existing.id }, data: { isPublished: true, state: 'PUBLISHED' }, select: COURSE_FULL_SELECT });
    return { data: normalizeCourse(updated) };
  }

  async unpublish(idOrSlug: string) {
    const { data: existing } = await this.findOne(idOrSlug);
    const updated = await (this.prisma.course as any).update({ where: { id: existing.id }, data: { isPublished: false, state: 'DRAFT' }, select: COURSE_FULL_SELECT });
    return { data: normalizeCourse(updated) };
  }
  ...
  async duplicate(idOrSlug: string) {
    ...
    const copy = await this.prisma.$transaction(async (tx) => {
      ...
      for (const [si, section] of ((sections ?? []) as any[]).entries()) {
        const sec = await (tx as any).section.create({ data: { courseId: newCourse.id, title: section.title, order: section.order ?? si } });
        ...
        for (const [li, lesson] of ((section.lessons ?? []) as any[]).entries()) {
          await (tx as any).lesson.create({
            data: { sectionId: sec.id, courseId: newCourse.id, moduleId: mid, title: lesson.title, order: lesson.order ?? li, content: lesson.content ?? '', videoUrl: lesson.videoUrl ?? null },
          });
        }
      }
      return newCourse;
  ...
  async getCourseLabs(courseId: string) {
    const course = await this.prisma.course.findUnique({
      ...
      include: {
        courseLabs: {
          orderBy: { order: 'asc' },
          include: { lab: { select: { id: true, slug: true, title: true, difficulty: true, isPublished: true } } },
        },
      },
    });
  ...
  async reorderLabs(courseId: string, order: string[]) {
    ...
    await Promise.all(order.map((labId, idx) => this.prisma.courseLab.updateMany({ where: { courseId, labId }, data: { order: idx } })));
    return this.getCourseLabs(courseId);
  }

  async attachLab(courseId: string, labId: string) {
    const existing = await this.prisma.courseLab.findUnique({ where: { courseId_labId: { courseId, labId } } });
  ...
  async detachLab(courseId: string, labId: string) {
    const record = await this.prisma.courseLab.findUnique({ where: { courseId_labId: { courseId, labId } } });
  ...
  async importJson(buffer: Buffer, metadata: ImportCourseDto) {
    let parsed: any;
    try { parsed = JSON.parse(buffer.toString('utf-8')); }
    catch { throw new BadRequestException('Invalid JSON file'); }
    ...
    const course = await this.prisma.course.create({
      data: {
        ...
        state, isPublished: isPublishedFromState(state),
    ...
    if (parsed.topics && Array.isArray(parsed.topics) && parsed.topics.length > 0) {
      try { await this.saveCurriculum(course.id, parsed.topics); }
      catch (e: any) { console.warn(`[ImportJSON] saveCurriculum failed:`, e?.message); }
    }
