...
export class Lab2Service {
  ...
  async initLab(userId: string, labId: string) {
    ...
    await this.flagRecord.generateAndStore(userId, result.labId, 'lab2-attempt', result.dynamicFlag, 24);
  ...
  async getCapture(userId: string, labId: string) {
    ...
    const packets = [
      {
        no: 1, time: '0.000000',
        source: '10.10.0.22', destination: '10.10.0.5',
        protocol: 'TCP', length: 74, tcpStream: 0,
        info: '58142 → 8080 [SYN] Seq=0 Win=64240 Len=0',
      },
      {
        no: 2, time: '0.000620',
        source: '10.10.0.5', destination: '10.10.0.22',
        protocol: 'TCP', length: 74, tcpStream: 0,
      ...
      },
      {
        no: 3, time: '0.001100',
        source: '10.10.0.22', destination: '10.10.0.5',
        protocol: 'TCP', length: 66, tcpStream: 0,
        info: '58142 → 8080 [ACK] Seq=1 Ack=1 Win=64240 Len=0',
      },
      {
        no: 4, time: '0.001830',
        source: '10.10.0.22', destination: '10.10.0.5',
        protocol: 'HTTP', length: 286, tcpStream: 0,
      ...
      },
      {
        no: 5, time: '0.018400',
        source: '10.10.0.5', destination: '10.10.0.22',
        protocol: 'HTTP', length: 744, tcpStream: 0,
  ...
  async submitFlag(userId: string, labId: string, submittedFlag: string) {
    ...
    const resolvedLabId = await this.stateService.resolveLabId(labId);
    const result = await this.flagRecord.verifyAndConsume(userId, resolvedLabId, 'lab2-attempt', submittedFlag.trim());
    if (result === 'correct') {
      return {
        ...
        explanation:
          ...
          'session token over unencrypted HTTP. Base64 is encoding, not encryption.',
      };
    }
