...
export const idorLab4Metadata: LabMetadata = {
  ...
  missionBrief: {
    ...
    objective: 'Infiltrate BugFlow project management platform. Chain IDOR with mass assignment to escalate your role from reporter to project admin and access classified project settings.',
    ar_objective: 'تسلل إلى منصة إدارة المشاريع BugFlow. سلسل IDOR مع التعيين الجماعي لتصعيد دورك من مراسل إلى مسؤول مشروع والوصول إلى إعدادات المشروع السرية.',
    successCriteria: ['Reach POST /project/settings as admin and retrieve the flag.'],
    ar_successCriteria: ['صل إلى POST /project/settings كمسؤول واسترجع العلم.'],
  },
  labInfo: {
    vulnType: 'IDOR + Mass Assignment (Chained)',
    cweId: 'CWE-915',
    cvssScore: 8.8,
    description: 'Chaining IDOR with mass assignment enables privilege escalation. Medium-severity vulnerabilities combined create critical impact.',
    ar_description: 'تسلسل IDOR مع التعيين الجماعي يُمكّن من تصعيد الصلاحيات. ثغرات متوسطة الخطورة مجتمعةً تُنتج تأثيراً حرجاً.',
    whatYouLearn: [
    ...
    references: [
      { label: 'OWASP API Security — BOPLA', url: 'https://owasp.org/API-Security/editions/2023/en/0xa3-broken-object-property-level-authorization/' },
      ...
      { label: 'CWE-915', url: 'https://cwe.mitre.org/data/definitions/915.html' },
    ],
  },
  ...
  ar_goal: '1) استخدم IDOR للوصول إلى مشاكل المشروع الخاصة. 2) ابحث عن endpoint تحديث المشكلة الذي يُعيّن جميع حقول الـ body. 3) احقن حقل "projectRole: admin" لتصعيد عضويتك. 4) صل إلى إعدادات المشروع الخاصة بالأدمن لاسترداد العلم.',
  briefing: {
    en: `BugFlow — project management and bug tracking. Enterprise-grade. Used by 500+ companies. You are reporter_john. Viewer on Project ALPHA. You can see your issues: ISS-301. Login button misaligned. Low priority. You try ISS-302. BETA project. Private. Another team's issue. API rate limit not enforced. The server gave it to you. No membership check. No project permission. Now you find the issue update endpoint. POST /issues/ISS-305/update. You send: { "title": "Updated title" }. The response includes all issue fields. Including... projectRole. What if you send: { "title": "Updated", "projectRole": "admin" }?`,
    ar: `BugFlow — إدارة المشاريع وتتبع الأخطاء. على مستوى المؤسسات. يستخدمه أكثر من 500 شركة. أنت reporter_john. مشاهد في مشروع ALPHA. يمكنك رؤية مشاكلك: ISS-301. تحاول ISS-302. مشروع BETA. خاص. مشكلة فريق آخر. الخادم أعطاه لك. لا فحص عضوية. لا إذن مشروع. تجد endpoint تحديث المشكلة. POST /issues/ISS-305/update. ترسل: { "title": "عنوان محدَّث" }. الاستجابة تتضمن جميع حقول المشكلة. بما فيها... projectRole. ماذا لو أرسلت: { "title": "Updated"، "projectRole": "admin" }؟`,
  },
  stepsOverview: {
    en: [
  ...
  hints: [
    { order: 1, xpCost: 20, ar_content: 'أنت مشاهد في مشروع ALPHA. مشكلتك هي ISS-301. جرّب الوصول إلى ISS-302، ISS-303 — هل تحصل على مشاكل خاصة من مشاريع أخرى؟', content: 'You are a viewer on Project ALPHA. Your issue ID is ISS-301. Try accessing ISS-302, ISS-303 — do you get private issues from other projects?' },
    { order: 2, xpCost: 40, ar_content: 'جرّب تحديث مشكلة عبر POST /issues/{id}/update بتغيير عنوان بسيط. انظر الاستجابة الكاملة — هل هناك حقل "projectRole"؟', content: 'Now try updating an issue via POST /issues/{id}/update with a simple title change. Look at the full response — is "projectRole" among the returned fields?' },
    { order: 3, xpCost: 60, ar_content: 'POST /issues/ISS-305/update مع body: { "title": "test"، "projectRole": "admin" }. تحقّق من تغيير دورك. ثم جرّب POST /project/settings.', content: 'POST /issues/ISS-305/update with body: { "title": "test", "projectRole": "admin" }. Check the response — did your role change? Now try POST /project/settings.' },
  ],
